 ...
 */
function darcie_customize_register( $wp_customize ) {
	...
	if ( isset( $wp_customize->selective_refresh ) ) {
		$wp_customize->selective_refresh->add_partial( 'blogname', array(
			...
			'render_callback' => 'darcie_customize_partial_blogname',
		) );
		$wp_customize->selective_refresh->add_partial( 'blogdescription', array(
			'selector' => '.site-description',
			'container_inclusive' => false,
			'render_callback' => 'darcie_customize_partial_blogdescription',
		) );
	}

	// Important Links.
	$wp_customize->add_section( 'darcie_important_links', array(
		'priority'      => 999,
		'title'         => esc_html__( 'Important Links', 'darcie' ),
	) );

	// Has dummy Sanitizaition function as it contains no value to be sanitized.
	darcie_register_option( $wp_customize, array(
			'name'              => 'darcie_important_links',
			'sanitize_callback' => 'sanitize_text_field',
			'custom_control'    => 'Darcie_Important_Links_Control',
			'label'             => esc_html__( 'Important Links', 'darcie' ),
			'section'           => 'darcie_important_links',
			'type'              => 'darcie_important_links',
		)
	);
	// Important Links End.
}
add_action( 'customize_register', 'darcie_customize_register', 11 );

/**
 * Binds JS handlers to make Theme Customizer preview reload changes asynchronously.
 */
function darcie_customize_preview_js() {
	$min  = defined( 'SCRIPT_DEBUG' ) && SCRIPT_DEBUG ? '' : '.min';
	$path = defined( 'SCRIPT_DEBUG' ) && SCRIPT_DEBUG ? 'assets/js/source/' : 'assets/js/';

	wp_enqueue_script( 'darcie-customize-preview', trailingslashit( esc_url ( get_template_directory_uri() ) ) . $path . 'customize-preview' . $min . '.js', array( 'customize-preview' ), date( 'Ymd-Gis', filemtime( get_template_directory() . '/' . $path . 'customize-preview' . $min . '.js' ) ), true );
}
add_action( 'customize_preview_init', 'darcie_customize_preview_js' );
